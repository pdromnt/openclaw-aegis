...
import type { OpenClawConfig, ChannelConfig } from './types';
import {
  MaskedInput,
  FormField,
  SelectField,
          ...
          {isUnknown ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {/* Streaming */}
                {tmpl.supportsStreaming && tmpl.streamingModes && (
                  <FormField label={t('config.streaming')}>
                    <SelectField
                      value={channelConfig.streaming ?? tmpl.defaultStreaming ?? ''}
                      onChange={(v) => updateChannel({ streaming: v || undefined })}
...
function PickChannelStep({ onPick, onClose, existingIds }: PickChannelStepProps) {
  const { t } = useTranslation();
