...
function AgentRow({ agent, index, onChange, onRemove }: AgentRowProps) {
  ...
  const updateAgent = (patch: Partial<AgentConfig>) => {
    onChange((prev) => {
      ...
      return { ...prev, agents: { ...prev.agents, list } };
    });
  };
        ...
        <div className={clsx(
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <FormField label={t('config.agentName')}>
              <TextField
                value={agent.name ?? ''}
                onChange={(v) => updateAgent({ name: v || undefined })}
            ...
            <FormField label={t('config.primaryModel')} hint={t('config.inheritedFromDefaults')}>
              <TextField
                ...
                onChange={(v) =>
                  updateAgent({ model: { ...agent.model, primary: v || undefined } })
                }
          ...
          <FormField label={t('config.workspace')} hint={t('config.inheritedFromDefaults')}>
            <TextField
              ...
              onChange={(v) => updateAgent({ workspace: v || undefined })}
              placeholder={t('config.inheritedFromDefaults')}
            />
          </FormField>
            ...
            {heartbeatEnabled && (
              <FormField label={t('config.heartbeatEvery')} hint="e.g. 30m, 1h, 45m">
                <TextField
                  ...
                  onChange={(v) =>
                    updateAgent({ heartbeat: { ...agent.heartbeat, every: v } })
                  }
                  placeholder="30m"
                />
              </FormField>
          ...
          <div className="flex items-center justify-between">
            ...
            <ToggleSwitch
              value={isDefault}
              onChange={(v) => updateAgent({ isDefault: v || undefined })}
            />
          </div>
...
function AddAgentModal({ onClose, onAdd }: AddAgentModalProps) {
  ...
  return (
    ...
    >
      <div
        className={clsx(
          'bg-aegis-card border border-aegis-border rounded-2xl w-full max-w-md',
